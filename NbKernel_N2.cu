...
// |-> N_orbitals is just N-bodies (moving away from using #DEFINE's)
{
    ...
    float3 force = computeOrbitalForces(curPos, oldPos, N, multiThreaded);
...
//============================================================//
__device__ float3
computeOrbitalForces(float4 orbPos, float4* positions, int N, bool multiThreaded)
{
    /* Declaring this as external & shared means that all threads are able
     * to access it, and it makes me happy as they all get along and I don't
     * have to write more complicated shared memory access :D
    ...
    // TODO: NOTE !! MULTITHREADING DOES NOT WORK, UNEXPECTED RESULTS OCCUR
    if (multiThreaded)
    {
        SH_M_SUM(threadIdx.x, threadIdx.y).x = force.x;
        SH_M_SUM(threadIdx.x, threadIdx.y).y = force.y;
        SH_M_SUM(threadIdx.x, threadIdx.y).z = force.z;
        ...
        if (threadIdx.y == 0)
        {
            ...
            {
                force.x += SH_M_SUM(threadIdx.x, i).x;
                force.y += SH_M_SUM(threadIdx.x, i).y;
                force.z += SH_M_SUM(threadIdx.x, i).z;
            }
        }
    }
